 ...
 */
package com.unistpixel.unistpixeldungeon.ui;
...
import com.unistpixel.unistpixeldungeon.scenes.CellSelector;
...
import com.unistpixel.unistpixeldungeon.windows.WndCatalogs;
...
public class Toolbar extends Component {
	...
	protected void layout() {
		...
		int slots = UNISTPixelDungeon.quickSlots();
		...
		for(int i = 0; i <= 3; i++) {
			...
			if (slots == 4 && width < 150){
				if (width < 139){
					if ((UNISTPixelDungeon.flipToolbar() && i == 3) ||
							(!UNISTPixelDungeon.flipToolbar() && i == 0)) {
					...
					if (i == 0 && !UNISTPixelDungeon.flipToolbar() ||
						i == 3 && UNISTPixelDungeon.flipToolbar()){
						btnQuick[i].border(0, 2);
						btnQuick[i].frame(106, 0, 19, 24);
					} else if (i == 0 && UNISTPixelDungeon.flipToolbar() ||
							i == 3 && !UNISTPixelDungeon.flipToolbar()){
		...
		float right = width;
		switch(Mode.valueOf(UNISTPixelDungeon.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
		...
		if (UNISTPixelDungeon.flipToolbar()) {
