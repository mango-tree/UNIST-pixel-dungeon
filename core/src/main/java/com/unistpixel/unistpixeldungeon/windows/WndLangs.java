...
import com.unistpixel.unistpixeldungeon.scenes.TitleScene;
...
public class WndLangs extends Window {
	...
	public WndLangs(){
		...
		for (int i = 0; i < langs.size(); i++){
			...
			RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName())){
				...
				protected void onClick() {
					...
					UNISTPixelDungeon.switchNoFade(TitleScene.class, new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							UNISTPixelDungeon.language(langs.get(langIndex));
			...
			if (UNISTPixelDungeon.landscape() && i % 2 == 1){
		...
		resize(UNISTPixelDungeon.landscape() ? WIDTH_L : WIDTH_P, y);
