...
public class WndTextInput extends Window {
	...
	public WndTextInput(final String title, final String initialValue, final int maxLength,
		...
		final int width;
		if (UNISTPixelDungeon.landscape() && multiLine){
			width = W_LAND_MULTI; //more editing space for landscape users
		} else {
			width = WIDTH;
		}

		UNISTPixelDungeon.instance.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				...
				textInput = new EditText(UNISTPixelDungeon.instance);
				...
				UNISTPixelDungeon.instance.addContentView(textInput, layout);
			}
		});
	}
		...
		if (textInput != null){
			UNISTPixelDungeon.instance.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					InputMethodManager imm = (InputMethodManager) UNISTPixelDungeon
					...
					UNISTPixelDungeon.updateImmersiveMode();
