...
import com.unistpixel.unistpixeldungeon.effects.BannerSprites.Type;
...
import com.unistpixel.unistpixeldungeon.windows.WndChallenges;
import com.unistpixel.unistpixeldungeon.windows.WndClass;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		curClass = null;
		updateClass( HeroClass.values()[UNISTPixelDungeon.lastClass()] );
		...
		Badges.loadingListener = new Callback() {
			...
			public void call() {
				if (Game.scene() == StartScene.this) {
					UNISTPixelDungeon.switchNoFade( StartScene.class );
				}
			}
		};
	}
	...
	private class ChallengeButton extends Button {
		...
		protected void createChildren() {
			...
			image = Icons.get( UNISTPixelDungeon.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF );
			add( image );
		}
		...
		protected void onClick() {
			if (Badges.isUnlocked( Badges.Badge.VICTORY )) {
				StartScene.this.add(new WndChallenges(UNISTPixelDungeon.challenges(), true) {
					public void onBackPressed() {
						super.onBackPressed();
						image.copy( Icons.get( UNISTPixelDungeon.challenges() > 0 ?
